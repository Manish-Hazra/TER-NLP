...
let hyp=document.getElementById("hypothesis").value;
let res=document.getElementById("result");
let finalhyp=document.getElementById("finalhypothesis");
...
function shifting_op(hyp, ref) {
  ...
  for(let i=0; i<hypothesis_length; i++) {
    for(let j=0; j<reference_length; j++) {
      if (i!=j&&hyp[i]===ref[j]) {
        let len=0;
        for(let p=i, q=j; p<hypothesis_length&&q<reference_length; p++, q++) {
          if(hyp[p]!=ref[q]) 
            break;
          len+=1;
        }
  ...
  for (let i=0; i<sentences.length; i++) {
    ...
    score.push({val: edit_distance_original - resultant, hypothesis: sentences[i]});
  }
  if(score.length==0) {
    return {val: 0, hypothesis: hyp};
  }
  ...
  if(sortedScore[0]["val"]==0) {
    ...
    return {val: sortedScore[0].val, hypothesis: sortedScore[0].hypothesis};
  }
}

function TER(e) {
  e.preventDefault();
  let refr=document.getElementById("reference").value; 
  let hypo=document.getElementById("hypothesis").value;
  let card=document.getElementById("crd");
  let hyp=hypo.split(" "); let ref=refr.split(" "); let edit=0;
  while(1) {
